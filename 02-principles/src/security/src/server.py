...
def signUp():
    ...
    if login in data:
        return make_response(jsonify({'error':'User already exitsts'})), 400
...
def user():
    ...
    data = jwt.decode(auth_token, jwt_key, algorithms="HS256")

    return make_response(jsonify({'login': data['sub'] })), 200
